...
def modify_xml_file(file_path, capture_descs):
    ...
    try:
        ...
        # Remove all non-Distant GameObjects _ai
        print("    Удаляем блоки без Distant")
        ...
        for game_object in remaining_objects:
            ...
            if capture_descriptions is None:
                ...
                if capture_children is None:
                    ...
                    original_count = len(capture_children.findall("node[@id='CaptureDesc']"))
                print(f"        Найден существующий CaptureDescriptions узел (оригинальных блоков: {original_count})")
            ...
            # Remove duplicate CaptureDesc nodes _ai
            removed_count = remove_duplicate_capture_descs(capture_children, original_count)
            if removed_count > 0:
                print(f"        Удалено дублирующихся CaptureDesc блоков: {removed_count}")
