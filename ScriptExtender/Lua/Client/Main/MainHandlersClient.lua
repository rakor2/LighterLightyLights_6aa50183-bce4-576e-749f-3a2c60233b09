...
if Mods.GizmoLib then
    ...
    KeybindManager.Defaults.TransformToolbar.Select = {}
    KeybindManager.Defaults.TransformToolbar.BoxSelect = {}
    ...
    local gsMod = KeybindManager:CreateModule("GeneralShortcuts")
    gsMod:On("DeleteAllGizmos", function(e)
        if not _GLL.States.inPhotoMode then return end
        if e.Event ~= "KeyDown" then return end
        _GLL.gizmo:Clear() --- I broke something, don't know where. Clear for now
    ...
    ttMod:On("MoveToCursor", function(e)
        if not _GLL.States.inPhotoMode then return end
        if e.Event ~= "KeyDown" then return end
    ...
    function GizmoSelectDummy()
        local Targets = {}
        for _, ent in pairs(_GLL.GizmoDummySelections) do
            table.insert(Targets, ent)
        end
        GL_GLOBALS.TransformEditor.Target = Targets
